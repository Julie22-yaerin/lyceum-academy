...
export async function generateGraph(topic: string) {
  ...
  const res = await fetch(`${API_BASE}/ai/topic-map`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function getNodeSummary(concept: string) {
  ...
  const res = await fetch(`${API_BASE}/ai/node-summary`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function uploadProblemSet(file: File) {
  ...
  try {
    const res = await fetch(`${API_BASE}/ai/upload-pset`, { method: 'POST', body: form, signal: controller.signal });
    if (!res.ok) {
      const body = await res.text().catch(() => '');
      throw new Error(`Backend ${res.status}: ${body || res.statusText}`);
    }
...
export async function decomposeProblemSet(text: string) {
  ...
  const res = await fetch(`${API_BASE}/ai/decompose`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function checkMastery(question: string, answer: string) {
  ...
  const res = await fetch(`${API_BASE}/ai/mastery`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function validateToolMap(columns: object) {
  ...
  const res = await fetch(`${API_BASE}/ai/tool-map/validate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function cleanQuestion(prompt: string, context = '') {
  const res = await fetch(`${API_BASE}/ai/clean-question`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function synthesizeNoteFromUrl(url: string): Promise<NoteResult> {
  const res = await fetch(`${API_BASE}/ai/note`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function synthesizeNoteFromFile(file: File): Promise<NoteResult> {
  ...
  try {
    const res = await fetch(`${API_BASE}/ai/note-upload`, { method: 'POST', body: form, signal: controller.signal });
    if (!res.ok) {
      const body = await res.text().catch(() => '');
      throw new Error(`Backend ${res.status}: ${body || res.statusText}`);
    }
...
export async function analyzePage(pageData: string, pageIndex: number, totalPages: number) {
  const res = await fetch(`${API_BASE}/ai/analyze-page`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function gradeDual(
  ...
  const res = await fetch(`${API_BASE}/ai/grade-dual`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function gradeAll(questions: { id: string; prompt: string; answer: string }[]) {
  const res = await fetch(`${API_BASE}/ai/grade-all`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ questions }),
  });
...
export async function analyzeOnboarding(answers: Record<string, string | string[]>) {
  const res = await fetch(`${API_BASE}/ai/onboarding-analyze`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function feynmanTest(
  ...
  try {
    const res = await fetch(`${API_BASE}/ai/feynman`, {
      method: 'POST',
      body: form,
...
export async function noteChatMessage(
  ...
  noteContext: { title: string; summary: string; key_concepts: { concept: string; definition: string; explanation: string }[]; key_insight?: string }
) {
...
export async function describeDrawing(imageData: string) {
  ...
  const res = await fetch(`${API_BASE}/ai/describe-drawing`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
